...
class CameraModel:
    ...
    def deserialize_calibration(self, data):
        ...
        if len(data) < header_size * 4:  # 4 bytes per int
            raise ValueError(f"Insufficient data size. Expected at least {header_size * 4} bytes.")
        ...
        required_data_size = (
            ...
            8 * (header[iK] + header[iD] + header[iR] + header[iP]) +  # Double precision matrices
        ...
        if header[iL] != 0:
            ...
            self.local_transform = np.frombuffer(local_transform_bytes, dtype=np.float32)
            index += 4 * 12
